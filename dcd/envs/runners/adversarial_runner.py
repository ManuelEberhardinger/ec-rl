...
try:
    from dreamcoder.domains.minigrid.main import DreamCoder
except:
    print('Could not import dreamcoder! Can be ignored if running form dcd directory.')
...
class AdversarialRunner(object):
    ...
    def __init__(
        ...
        self.data_iter_file = 0
        timestamp = timestamp = datetime.datetime.now().isoformat()
        log_dir = '/home/ma/e/eberhardinger/workspaces/ec/dreamcoder/domains/minigrid/collected_data'
        self.collected_data_dir = os.path.join(log_dir, timestamp)
        if not os.path.exists(self.collected_data_dir):
            os.makedirs(self.collected_data_dir, exist_ok=True)

        self.level_dir = os.path.join(self.collected_data_dir, 'level_sampler')
        ...
        self.dreamcoder = DreamCoder(self.venv, self.ued_venv, self.level_samplers, self.level_store, args.log_dir, args.num_processes)
        # Set up ALP-GMM
        if self.is_alp_gmm:
            self._init_alp_gmm()
        ...
        for step in range(num_steps):
            ...
            if collect_data:
                for p_id in range(args.num_processes):
                    collected_data.append([step, p_id, obs_id['image'][p_id].cpu().numpy(), obs_id['direction'][p_id].cpu().item(),
                                           obs['image'][p_id].cpu().numpy(), obs['direction'][p_id].cpu().item(), 
                                           action[p_id].cpu().item(), reward[p_id].cpu().item(),
                                           done[p_id].item(), current_level_seeds[p_id].item()])
            if level_sampler and level_replay:
        ...
        if level_replay and edit_level:
            ...
            if self.base_levels == 'batch':
                ...
                if args.num_processes >= 4:
                    # take top 4
                    easy = list(np.argsort((agent_info['mean_return'].detach().cpu().numpy(
                    ) - agent_info['batched_value_loss'].detach().cpu().numpy()))[:4])
                    fixed_seeds = [env_info[x.item()] for x in easy] * int(args.num_processes/4)
                else:
                    # take top 1
                    easy = np.argmax((agent_info['mean_return'].detach().cpu().numpy() -
                                      agent_info['batched_value_loss'].detach().cpu().numpy()))
                    fixed_seeds = [env_info[easy]] * args.num_processes

            level_sampler, is_updateable = self._get_level_sampler('agent')

            # Before editing levels we run the agent on the easiest levels
            # so that we can take them as example for DreamCoder
            agent_data = self.agent_rollout(
                agent=agent,
                num_steps=self.agent_rollout_steps,
                update=False,
                level_replay=False,
            ...
            if self.data_iter_file % 500 == 0 and self.data_iter_file > 0:
                print('run dreamcoder')
                self.dreamcoder.run(agent_data)
        
            # we increment the counter for the iter file anyways so that we know every fifth step
            self.data_iter_file += 1
