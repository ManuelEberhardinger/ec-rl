...
class MinigridMazeFeatureExtractorBase(DeviceAwareModule):
    ...
    def featuresOfTask(self, t):
        inp = {
            ...
            'action':  torch.tensor([out for (inp, out) in t.examples], dtype=torch.float),
        }
        return self(inp)
...
class MinigridMazeFeatureExtractor(MinigridMazeFeatureExtractorBase):
    ...
    def __init__(self, tasks, obs_shape=(1, 5, 5),
        ...
        action_dim = 7
        self.action_embed = nn.Linear(action_dim, scalar_fc)
    ...
    def forward(self, inputs):
        ...
        in_action = F.one_hot(action.long(), num_classes=self.action_dim).to(self.device)
        in_action = self.action_embed(in_action.float())
        in_embedded = torch.cat((in_image, in_direction, in_action), axis=-1)
        in_embedded = torch.sum(in_embedded, axis=0).unsqueeze(0)
        return in_embedded
