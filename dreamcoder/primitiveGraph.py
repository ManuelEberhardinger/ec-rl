...
def graphPrimitivesFromGrammar(grammars, prefix, view=False):
    try:
    ...
    primitives = {p
                  ...
                  if p.isInvented}
    age = {p: min(j for j, g in enumerate(grammars) if p in g.primitives) + 1
           for p in primitives}
    ...
    def makeGraph(ordering, fn):
        ...
        if False:
            with g.subgraph(name='cluster_0') as sg:
                ...
                for j, primitive in enumerate(grammars[-1].primitives):
