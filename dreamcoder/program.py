...
class Program(object):
    ...
    def parse(s):
        ...
        def p(e):
            ...
            if e in InventedWithName.GLOBALS:
                return InventedWithName(e, InventedWithName.GLOBALS[e])
            if e == '??' or e == '?':
    ...
    def evaluate(self, environment):
        if self.isConditional:
            ...
            if self.branch.evaluate(environment):
                e = self.trueBranch.evaluate(environment)
                print('true', e, self.trueBranch)
                ef = self.falseBranch.evaluate(environment)
                print('false', ef, self.falseBranch, environment)
                return e
            else:
                e = self.falseBranch.evaluate(environment)
                print('false', e)
                return e
        else:
...
class InventedWithName(Invented):
    GLOBALS = {}
    ...
    def __init__(self, name, body):
        ...
        self.name = name
        self.hashCode = None
        InventedWithName.GLOBALS[self.name] = body

    def __eq__(self, o): return isinstance(
        o, InventedWithName) and o.name == self.name
    ...
    def clone(self): return InventedWithName(self.name, self.body)

    @staticmethod
    def _parse(s, n):
        print('parse InventedWithName')
        while n < len(s) and s[n].isspace():
            n += 1
        name = []
        while n < len(s) and not s[n].isspace() and s[n] not in '()':
            ...
            n += 1
        name = "".join(name)
        if name in InventedWithName.GLOBALS:
            eprint(f'InventedWithName return {name} ')
            return InventedWithName(name, InventedWithName.GLOBALS[name]), n
        raise ParseFailure(s)
