...
class ECResult():
    ...
    def __getstate__(self):
        ...
        if 'parameters' in state and 'dcd' in state['parameters']:
            del state['parameters']['dcd']
        return state
...
def ecIterator(grammar, tasks,
    ...
    if resume is not None:
        ...
        result.parameters['dcd'] = dcd
        # only for my work not official...
        # test as the tasks changes
        result.allFrontiers = {
            t: Frontier([],
    ...
    if addFullTaskMetrics:
        ...
        if result.recognitionModel is not None:
            _enumerator = lambda *args, **kw: result.recognitionModel.enumerateFrontiers(
                *args, **kw)
        else:
        ...
        updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                                 recognizer.taskGrammarLogProductions(tasks), 'taskLogProductions')
        updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                                 recognizer.taskGrammarEntropies(tasks), 'taskGrammarEntropies')
        updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                                 result.recognitionModel.taskAuxiliaryLossLayer(tasks), 'taskAuxiliaryLossLayer')

        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, testingTimes, 'heldoutTestingTimes')
        updateTaskSummaryMetrics(result.recognitionTaskMetrics, recognizer.taskGrammarLogProductions(
            testingTasks), 'heldoutTaskLogProductions')
        updateTaskSummaryMetrics(result.recognitionTaskMetrics, recognizer.taskGrammarEntropies(
            testingTasks), 'heldoutTaskGrammarEntropies')
        updateTaskSummaryMetrics(result.recognitionTaskMetrics, result.recognitionModel.taskAuxiliaryLossLayer(
            testingTasks), 'heldoutAuxiliaryLossLayer')
    ...
    for j in range(resume or 0, iterations):
        ...
        if useRecognitionModel and biasOptimal and helmholtzRatio > 0 and \
           all(str(p) != "REAL" for p in grammar.primitives):  # real numbers don't support this
            ...
            def helmholtzFrontiers(): return []

        reportMemory()

        # Get waking task batch.
        wakingTaskBatch = taskBatcher.getTaskBatch(
            result, tasks, taskBatchSize, j)
        ...
        if useDSL:
            ...
            topDownFrontiers, times = wake_generative(grammar, wakingTaskBatch,
                                                      ...
                                                      CPUs=CPUs,
                                                      evaluationTimeout=evaluationTimeout)
            result.trainSearchTime = {t: tm for t,
                                      tm in times.items() if tm is not None}
        else:
            eprint(
        ...
        for f in topDownFrontiers:
            ...
            result.allFrontiers[f.task] = result.allFrontiers[f.task].combine(
                f).topK(maximumFrontier)

        eprint("Frontiers discovered top down: " + str(len(tasksHitTopDown)))
        eprint("Total frontiers: " +
               str(len([f for f in result.allFrontiers.values() if not f.empty])))
        stats.append({
            'tasks': len(wakingTaskBatch),
            'tasksHitTopDown': len(tasksHitTopDown)
        })
        ...
        if useRecognitionModel:
            ...
            tasksHitBottomUp = \
                sleep_recognition(result, grammar, wakingTaskBatch, tasks, testingTasks, result.allFrontiers.values(),
                                  ...
                                  recognitionSteps=recognitionSteps, maximumFrontier=maximumFrontier, dcd=dcd, env_name=env_name)

            showHitMatrix(tasksHitTopDown, tasksHitBottomUp, wakingTaskBatch)
            stats[-1]['tasksHitBottomUp'] = len(tasksHitBottomUp)
            stats[-1]['totalHits'] = len(tasksHitBottomUp | tasksHitTopDown)
...
def evaluateOnTestingTasks(result, testingTasks, grammar, _=None,
    ...
    print(grammar.primitives)
    if result.recognitionModel is not None:
        eprint('use recognitionModel')
        recognizer = result.recognitionModel
        ...
        eprint('no recognitionModel in ckpt')
    ...
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             {f.task: f for f in testingFrontiers if len(
                                 f) > 0},
                             'frontier')
    for f in testingFrontiers:
        result.recordFrontier(f)
    result.testSearchTime = {t: tm for t,
                             tm in times.items() if tm is not None}
...
def default_wake_generative(grammar, tasks,
    ...
    summaryStatistics("Generative model", [
                      t for t in times.values() if t is not None])
...
def sleep_recognition(result, grammar, taskBatch, tasks, testingTasks, allFrontiers, _=None,
                      ...
                      auxiliaryLoss=None, cuda=None, CPUs=None, solver=None, dcd=None, env_name=None):
    eprint("Using an ensemble size of %d. Note that we will only store and test on the best recognition model." % ensembleSize)

    featureExtractorObjects = [featureExtractor(
        tasks, testingTasks=testingTasks, cuda=cuda, dcd=dcd, env_name=env_name) for i in range(ensembleSize)]
    ...
    eprint("Trained an ensemble of %d recognition models, now enumerating." %
    ...
    for recIndex, recognizer in enumerate(trainedRecognizers):
        eprint("Enumerating from recognizer %d of %d" %
               (recIndex, len(trainedRecognizers)))
        ...
        ensembleTimes.append(
            [t for t in allRecognitionTimes.values() if t is not None])
        ensembleRecognitionTimes.append(allRecognitionTimes)

        recognizerTasksHitBottomUp = {
    ...
    result.trainSearchTime = {tk: tm for tk, tm in ensembleRecognitionTimes[bestRecognizer].items()
                              if tm is not None}
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             ensembleRecognitionTimes[bestRecognizer], 'recognitionBestTimes')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskHiddenStates(tasks), 'hiddenState')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskGrammarLogProductions(tasks), 'taskLogProductions')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskGrammarEntropies(tasks), 'taskGrammarEntropies')
    if contextual:
        updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                                 result.recognitionModel.taskGrammarStartProductions(
                                     tasks),
                                 'startProductions')
    ...
    eprint("Cumulative results for the full ensemble of %d recognizers: " %
           len(trainedRecognizers))
    ...
    eprint("Frontiers discovered bottom up: " +
           str(len(totalTasksHitBottomUp)))
    eprint("Total frontiers: " +
           str(len([f for f in result.allFrontiers.values() if not f.empty])))
    ...
    if len(ensembleTimes[bestRecognizer]) > 0:
        eprint("Average search time: ", int(mean(ensembleTimes[bestRecognizer]) + 0.5),
               "sec.\tmedian:", int(
                   median(ensembleTimes[bestRecognizer]) + 0.5),
...
def consolidate(result, grammar, _=None, topK=None, arity=None, pseudoCounts=None, aic=None,
    ...
    if len([f for f in compressionFrontiers if not f.empty]) == 0:
        ...
        for c in compressionFrontiers:
            result.allFrontiers[c.task] = c.topK(
                0) if c in needToSupervise else c
...
def commandlineArguments(_=None,
    ...
    parser.add_argument("--biasOptimal",
                        ...
                        default=biasOptimal, action="store_true")
    ...
    if v["countParameters"]:
        ...
        eprint("The recognition model has",
               sum(p.numel()
                   for p in result.recognitionModel.parameters() if p.requires_grad),
               "trainable parameters and",
               sum(p.numel() for p in result.recognitionModel.parameters()),
               "total parameters.\n",
               "The feature extractor accounts for",
               sum(p.numel()
                   for p in result.recognitionModel.featureExtractor.parameters()),
               "of those parameters.\n",
               "The grammar builder accounts for",
               sum(p.numel()
                   for p in result.recognitionModel.grammarBuilder.parameters()),
               "of those parameters.\n")
        sys.exit(0)
...
def addTaskMetrics_(result, path):
    ...
    if hasattr(list(tasks)[0], 'getImage'):
        ...
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, images, 'taskImages')
    ...
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.auxiliaryPrimitiveEmbeddings(), 'auxiliaryPrimitiveEmbeddings')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskAuxiliaryLossLayer(tasks), 'taskAuxiliaryLossLayer')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskAuxiliaryLossLayer(everyTask), 'every_auxiliaryLossLayer')

    updateTaskSummaryMetrics(result.recognitionTaskMetrics, result.recognitionModel.taskGrammarFeatureLogProductions(
        tasks), 'grammarFeatureLogProductions')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics, result.recognitionModel.taskGrammarFeatureLogProductions(
        everyTask), 'every_grammarFeatureLogProductions')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskGrammarLogProductions(tasks), 'contextualLogProductions')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics, result.recognitionModel.taskGrammarLogProductions(
        everyTask), 'every_contextualLogProductions')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskHiddenStates(tasks), 'hiddenState')
    updateTaskSummaryMetrics(result.recognitionTaskMetrics,
                             result.recognitionModel.taskHiddenStates(everyTask), 'every_hiddenState')
    ...
    if False:
        eprint(
            f"About to do an expensive Monte Carlo simulation w/ {len(tasks)} tasks")
        updateTaskSummaryMetrics(result.recognitionTaskMetrics,
