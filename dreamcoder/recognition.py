...
class GrammarNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        logProductions = self.logProductions(x).squeeze()
    ...
    def batchedLogLikelihoods(self, xs, summaries):
        ...
        numerator = (
            logProductions * maybe_cuda(torch.from_numpy(uses).float(), use_cuda)).sum(1)
        numerator += maybe_cuda(torch.tensor(
            [summary.constant for summary in summaries]).float(), use_cuda)
        ...
        for tau in range(len(alternativeSet)):
            ...
            mask[tau, len(self.grammar)] = 0. if Index(
                0) in alternativeSet[tau] else NEGATIVEINFINITY
        ...
        z = mask.repeat(
            B, 1, 1) + logProductions.repeat(len(alternativeSet), 1, 1).transpose(1, 0)
        # z: BxR
        ...
        denominator = (maybe_cuda(torch.tensor(
            N).float(), use_cuda) * z).sum(1)
        # print(numerator)
        # print(denominator)
...
class ContextualGrammarNetwork_LowRank(nn.Module):
    def __init__(self, inputDimensionality, grammar, R=16):
        ...
        for prim in grammar.primitives:
            ...
            idx_list = list(
                range(self.n_grammars, self.n_grammars+numberOfArguments))
    ...
    def forward(self, x):
        assert len(
            x.size()) == 1, "contextual grammar doesn't currently support batching"
    ...
    def vectorizedLogLikelihoods(self, x, summaries):
        ...
        for b, summary in enumerate(summaries):
            ...
            for p, production in enumerate(self.grammar.primitives):
                uses[b, self.n_grammars - 1,
                     p] = summary.noParent.uses.get(production, 0.)
            uses[b, self.n_grammars - 1, G -
                 1] = summary.noParent.uses.get(Index(0), 0.)

            # variableParent: this is the penultimate network output
            for p, production in enumerate(self.grammar.primitives):
                uses[b, self.n_grammars - 2,
                     p] = summary.variableParent.uses.get(production, 0.)
            uses[b, self.n_grammars - 2, G -
                 1] = summary.variableParent.uses.get(Index(0), 0.)
        ...
        constant = np.zeros(B)
        for b, summary in enumerate(summaries):
            constant[b] += summary.noParent.constant + \
                summary.variableParent.constant
            for ss in summary.library.values():
                for s in ss:
                    constant[b] += s.constant

        numerator = numerator + \
            maybe_cuda(torch.tensor(constant).float(), use_cuda)
        ...
        for parent, index, alternatives in alternativeSet:
            ...
            entries = r[[primitiveColumn[alternative]
        ...
        normalizerVector = torch.cat(
            [alternativeNormalizer[k] for k in normalizerKeys])
    ...
    def batchedLogLikelihoods(self, xs, summaries):
        ...
        numerator = (
            logProductions*maybe_cuda(torch.tensor(uses).float(), use_cuda)).view(B, -1).sum(1)
        ...
        if True:
            ...
            for tau in range(len(alternativeSet)):
                for p, production in enumerate(self.grammar.primitives):
                    mask[tau, p] = 0. if production in alternativeSet[tau] else NEGATIVEINFINITY
                mask[tau, G -
                     1] = 0. if Index(0) in alternativeSet[tau] else NEGATIVEINFINITY
            mask = maybe_cuda(torch.tensor(mask).float(), use_cuda)
            ...
            for b, summary in enumerate(summaries):
                ...
                # noParent: this is the last network output
                for r, alternatives in enumerate(alternativeSet):
                    N[b, self.n_grammars - 1,
                        r] = summary.noParent.normalizers.get(alternatives, 0.)
                # variableParent: this is the penultimate network output
                for r, alternatives in enumerate(alternativeSet):
                    N[b, self.n_grammars - 2,
                        r] = summary.variableParent.normalizers.get(alternatives, 0.)
            N = maybe_cuda(torch.tensor(N).float(), use_cuda)
            denominator = (N*z).sum(1).sum(1)
        else:
            gs = [self(xs[b]) for b in range(B)]
            denominator = torch.cat([summary.denominator(g)
                                    for summary, g in zip(summaries, gs)])

        ll = numerator - denominator

        if False:  # verifying that batching works correctly
            gs = [self(xs[b]) for b in range(B)]
            _l = torch.cat([summary.logLikelihood(g)
                           for summary, g in zip(summaries, gs)])
...
class ContextualGrammarNetwork_Mask(nn.Module):
    def __init__(self, inputDimensionality, grammar):
        ...
        # We had an extra grammar for when there is no parent and for when the parent is a variable
        self.n_grammars += 2
        self._transitionMatrix = nn.Parameter(nn.init.xavier_uniform(
            torch.Tensor(self.n_grammars, len(grammar) + 1)))
        self._logProductions = nn.Linear(inputDimensionality, len(grammar)+1)

    def transitionMatrix(self, x):
        if len(x.shape) == 1:  # not batched
            # will broadcast
            return self._logProductions(x) + self._transitionMatrix
...
class ContextualGrammarNetwork(nn.Module):
    ...
    def batchedLogLikelihoods(self, xs, summaries):
        ...
        z = mask.repeat(self.n_grammars, 1, 1).repeat(B, 1, 1, 1) + \
            logProductions.repeat(len(alternativeSet), 1,
                                  1, 1).transpose(0, 1).transpose(1, 2)
        z = torch.logsumexp(z, 3)  # pytorch 1.0 dependency
...
class RecognitionModel(nn.Module):
    def __init__(self, featureExtractor, grammar, hidden=[64], activation="tanh",
        ...
        if self.contextual:
            ...
            self.grammarBuilder = GrammarNetwork(
                self.outputDimensionality, grammar)

        self.grammar = ContextualGrammar.fromGrammar(
            grammar) if contextual else grammar
        ...
        if previousRecognitionModel:
            self._MLP.load_state_dict(
                previousRecognitionModel._MLP.state_dict())
    ...
    def auxiliaryLoss(self, frontier, features):
        ...
        if self.use_cuda:
            u = u.cuda()
        preds = self._auxiliaryPrediction(features)
        al = self._auxiliaryLoss(preds.squeeze(), u)
        return al
    ...
    def auxiliaryPrimitiveEmbeddings(self):
        ...
        primitivesDict = {self.grammar.primitives[i]: auxiliaryWeights[i, :]
    ...
    def grammarLogProductionDistanceToTask(self, task, tasks):
        ...
        taskLogits = self.grammarLogProductionsOfTask(
            task).unsqueeze(0)  # Change to [1, D]
    ...
    def train(self, frontiers, _=None, steps=None, lr=0.001, topK=5, CPUs=1,
              ...
              helmholtzFrontiers=[], helmholtzRatio=0., helmholtzBatch=5000,
        ...
        def updateHelmholtzTasks():
            updateCPUs = CPUs if hasattr(
                self.featureExtractor, 'parallelTaskOfProgram') and self.featureExtractor.parallelTaskOfProgram else 1
            if updateCPUs > 1:
                eprint("Updating Helmholtz tasks with", updateCPUs, "CPUs",
        ...
        for i in range(1, epochs + 1):
            ...
            if totalGradientSteps > steps:
                print('totalGradientSteps > steps')
                break
            ...
            for frontier in permutedFrontiers:
                ...
                if is_torch_invalid(loss):
                    ...
                    descriptionLengths.append(
                        min(-e.logPrior for e in frontier))
                    if dreaming:
                    ...
                    if totalGradientSteps > steps:
                        print('2. totalGradientSteps > steps')
                        # Stop iterating, then print epoch and loss, then break to finish.
                        break

            if (i == 1 or i % 10 == 0) and losses:
                eprint("(ID=%d): " % self.id, "Epoch", i, "Loss", mean(losses))
                if realLosses and dreamLosses:
                    eprint("(ID=%d): " % self.id, "\t\t(real loss): ", mean(
                        realLosses), "\t(dream loss):", mean(dreamLosses))
                eprint("(ID=%d): " % self.id,
                       "\tvs MDL (w/o neural net)", mean(descriptionLengths))
                if realMDL and dreamMDL:
                    eprint("\t\t(real MDL): ", mean(realMDL),
                           "\t(dream MDL):", mean(dreamMDL))
                eprint("(ID=%d): " % self.id, "\t%d cumulative gradient steps. %f steps/sec" % (totalGradientSteps,
                                                                                                totalGradientSteps/(time.time() - start)))
                eprint("(ID=%d): " % self.id, "\t%d-way auxiliary classification loss" %
                       len(self.grammar.primitives), sum(classificationLosses)/len(classificationLosses))
                losses, descriptionLengths, realLosses, dreamLosses, realMDL, dreamMDL = [], [], [], [], [], []
                classificationLosses = []
                gc.collect()

        eprint("(ID=%d): " % self.id, " Trained recognition model in",
    ...
    def enumerateFrontiers(self,
        ...
        with timing("Evaluated recognition model"):
            ...
            grammars = {task: grammar.untorch()
                        for task, grammar in grammars.items() if grammar is not None}
    ...
    def taskOfProgram(self, p, tp):
        ...
        if random.random() < 0.5:
            ...
            while True:
                ...
                try:
                    y = runWithTimeout(lambda: p.runWithArguments(
                        xs), self.helmholtzEvaluationTimeout)
